...
def create_bert_embeddings(sentences):
    model = RepresentationModel(
        model_type="bert",
        model_name="bert-base-uncased",
        use_cuda=True,
    )
    sentence_vectors = model.encode_sentences(
        sentences, combine_strategy="mean", batch_size=512,
    )
    return sentence_vectors


def get_sentences(col_name, data):
    filtered_df = data[data[col_name].notnull()]
    l = set(filtered_df[col_name])
    return list(l)

def load_pickle(file='embeddings.pkl'):
    try:
        with open(file, "rb") as fIn:
            return pickle.load(fIn)
        print('loaded exisiting embedddings')
    except Exception as e:
        print(e)
        return {}
    
def store_pickle(data, file='embeddings.pkl'):
    with open(file, "wb") as fOut:
        pickle.dump(data, fOut)

def bert_embedding(sentences, cache_file='embeddings.pkl'):
    try:
        embeddings_map = load_pickle()
    except Exception as e:
        embeddings_map = {}
        print('cache not present, recreating the embeddings')

    embeddings = create_bert_embeddings(sentences)

    for i in range(len(corpus_sentences)):
        embeddings_map[sentences[i]] = corpus_embeddings[i].reshape(1, 768)
            
    store_pickle(corpus_embeddings_map)
    return corpus_embeddings_map

        
def get_bert_embedding(txt, cache=None, file='embeddings.pkl'):
    if cache and txt not in cache:
        cache[txt] = create_bert_embeddings([txt])
        store_pickle(cache)
        return cache[txt] 
    else:
        cache = load_pickle(file)
        if txt in cache:
            return cache[txt]
        else:
            cache[txt] = create_bert_embeddings([txt])
...
def process_movies(df_movies, use_embeddings=False):
    genres = set()
    ...
    def encode(movie_genres):
        ...
        for idx, genre in enumerate(genres):
            if genre in movie_genres:
                res.append(1)
                embed = get_bert_embedding(genre)[0]
        ...
        if use_embeddings:
            return res + [np.mean(embeddings, axis=0).tolist()]
        else:
